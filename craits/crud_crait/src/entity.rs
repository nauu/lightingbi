...
use sqlx::{Pool, Database, FromRow, Row, MySqlPool, MySql, Executor};
use std::fs::read;
...
use sqlx::Arguments;
use async_graphql::{OutputType, InputObject , SimpleObject};
...
impl MySqlRepository{
            ...
            T: for<'r> FromRow<'r, MySqlRow> + Send + Unpin + Debug + Entity,
    {
        ...
        let mut result = sqlx::query_as_with::<_ , T , MySqlArguments>(&sql_with_param , arg).fetch_all(pool).await?;
        Ok(result)
    }

    pub async fn query_page<T>(params:&BTreeMap<String , String> ,page_request:&PageRequest, pool : &MySqlPool) -> Result<Page<T>>
        where
            T: for<'r> FromRow<'r, MySqlRow> + Send + Unpin + Debug + Entity,{
        let table_name = T::table_name().await?;
        // total count
        ...
        let  context : Vec<T> = sqlx::query_as_with::<_ , T , MySqlArguments>(&sql_with_param , arg).fetch_all(pool).await?;
    ...
    pub async fn query_count(table_name:&String , params:&BTreeMap<String , String> , pool : &MySqlPool) -> Result<i64>{
        let mut sql = format!("select count(*) as count from {} " , table_name);
        let (mut sql_with_param, mut param_values) = Self::add_params_to_sql(sql , params).await?;
        let arg = Self::covert_param_values_to_arg(param_values).await?;
        let row:MySqlRow = sqlx::query_with::<_ , MySqlArguments>(&sql_with_param , arg).fetch_one(pool).await?;
    ...
    async fn covert_param_values_to_arg(params : Vec<&String>) -> Result<MySqlArguments> {
        ...
        if !params.is_empty(){
            params.iter().for_each(|v| {arg.add(v);()});
        }
        Ok(arg)
    }

    async fn add_params_to_sql(mut sql:String,params:&BTreeMap<String , String>) -> Result<(String,Vec<&String>)>{
    ...
    async fn covert_object_to_arg(params : Vec<&Value>) -> Result<MySqlArguments> {
        ...
        for value in params {
            ...
            match value {
                Value::Bool(_) => {arg.add(value.as_bool().unwrap_or(false));()}
                Value::Number(_) => {
                    if value.is_i64(){
                    ...
                    ()}
                Value::String(_) => {arg.add(value.as_str().unwrap_or(""));()}
                _ => ()
            } ;
        }
        Ok(arg)
            ...
            T: for<'r> FromRow<'r, MySqlRow> + Send + Unpin + Debug + Entity,
    {
        ...
        let insert_sql = format!("update {} set {} where {} = ? " , table_name , columns , id_name) ;
        println!("insert sql is : {}" , insert_sql);
        let arg = Self::covert_object_to_arg(values).await?;
        let rows_affected = sqlx::query_with(insert_sql.as_str() , arg)
            .execute( pool)
...
mod tests {
    ...
    use anyhow::Result;
    use async_trait::async_trait;
    use sqlx::mysql::{MySqlDatabaseError, MySqlArguments};
    use std::collections::BTreeMap;
    use sqlx::Arguments;
    ...
    async fn test_crud() -> Result<()>{
        ...
        // find
        let good1 = MySqlRepository::find_by_id::<Good>(&String::from("good_test") , &db_pool).await?.unwrap();
        println!("{:?}" , good1);
        ...
        // find
        let good2 = MySqlRepository::find_by_id::<Good>(&String::from("good_test") , &db_pool).await?.unwrap();
        println!("{:?}" , good2);
    ...
    async fn test_query_string() -> Result<()>{
        ...
        a.add("good1");
        a.add("2000");
        let mut cursor : Vec<Good> = sqlx::query_as_with::<_, Good, MySqlArguments>(sql, a).fetch_all(&db_pool).await?;
    ...
    async fn test_query_count()-> Result<()>{
        ...
        let goods = MySqlRepository::query_count(&"t_lighting_good".to_string() , &params , &db_pool).await?;
        println!("{:?}" , goods);
        Ok(())
    }
